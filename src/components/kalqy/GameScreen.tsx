...
export interface GameResult {
  stars: number;
  movements: Record<string, number>; // movement -> count correct
}
...
const ANIMALS: Animal[] = [
  { name: "Frog", emoji: "🐸", movement: "Jump", verb: "hop", skill: "balance" },
  { name: "Rabbit", emoji: "🐰", movement: "Crawl", verb: "crawl", skill: "coordination" },
  { name: "Elephant", emoji: "🐘", movement: "Squat", verb: "squat", skill: "bodyAwareness" },
  { name: "Duck", emoji: "🦆", movement: "Walk", verb: "waddle", skill: "coordination" },
];
...
export function GameScreen({ onBack, onComplete }: GameScreenProps) {
  ...
  const sequence = useMemo(() => {
    ...
    return out;
  }, [phase === "start"]); // regenerate when entering start
  ...
  useEffect(() => {
    if (phase === "playing" && current) {
      const t = setTimeout(() => {
        speak(`Can you ${current.verb} like a ${current.name}?`);
      }, 300);
  ...
  const handlePick = (animal: Animal) => {
    ...
    if (animal.name === current.name) {
      ...
      setConfetti(true);
      setStars((s) => s + 1);
      movementsRef.current[current.movement] =
        (movementsRef.current[current.movement] || 0) + 1;
      playTone(880, 0.15);
      ...
      setTimeout(() => {
        ...
        if (round + 1 >= TOTAL_ROUNDS) {
          setPhase("end");
          onComplete({ stars: stars + 1, movements: { ...movementsRef.current } });
        } else {
          setRound((r) => r + 1);
        }
      }, 1400);
    ...
    <div className="relative min-h-full overflow-hidden bg-gradient-to-b from-sky/40 via-leaf/30 to-leaf/60">
      ...
      <div className="relative mx-auto flex min-h-screen max-w-5xl flex-col px-4 py-6 md:px-8 md:py-8">
        ...
        <div className="mb-4 flex items-center justify-between">
          ...
          {phase === "playing" && (
            <div className="flex items-center gap-3 rounded-2xl bg-card/90 px-4 py-2 shadow-sm backdrop-blur">
    ...
    <div className="w-full">
      ...
      <div className="mb-6 flex flex-col items-center text-center">
        ...
        <h2 className="text-3xl font-black text-foreground drop-shadow-sm md:text-5xl">
          Can you {current.verb} like a {current.name}?
        </h2>
        {feedback === "correct" && (
          <div className="mt-4 animate-pop text-3xl font-black text-jungle">
            🌟 Great job! 🌟
          </div>
        )}
        {feedback === "wrong" && (
          <div className="mt-4 animate-pop text-2xl font-black text-coral">
            🐾 Try again!
          </div>
        )}
      </div>
...
function AnimalCard({
  ...
  return (
    ...
    >
      ...
      <div className="text-base font-black text-foreground md:text-lg">{animal.name}</div>
...
function EndScreen({
  ...
  return (
    <div className="animate-pop text-center">
      ...
      <h1 className="mb-2 text-4xl font-black text-foreground md:text-6xl">You did it!</h1>
      <p className="mb-6 text-lg font-bold text-foreground/80">You're a jungle superstar!</p>
        ...
        {Array.from({ length: TOTAL_ROUNDS }).map((_, i) => (
          <Star
            ...
            className={`h-12 w-12 transition-all ${
              i < stars ? "fill-sunshine text-sunshine animate-pop" : "text-muted"
            }`}
